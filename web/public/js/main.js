...
(function () {
  ...
  // Background photo (Ken Burns drift) + scrim + gradient orb field —
  // stacked behind everything, in back-to-front paint order.
  document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    let w, h, particles;
    const COUNT = 46;
    const COLORS = ['rgba(124,92,255,', 'rgba(76,231,255,', 'rgba(255,93,171,'];
    ...
    function step() {
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 130) {
            gfx.strokeStyle = `rgba(124,92,255,${0.08 * (1 - dist / 130)})`;
            gfx.lineWidth = 1;
            gfx.beginPath();
            gfx.moveTo(a.x, a.y);
            gfx.lineTo(b.x, b.y);
  ...
  /* ------------------------------------------------------------
     Entrance choreography — every floating element (nav links,
     pills, stat tiles, cards, buttons, the FAB) rises into place
     on load, staggered in reading order, then settles into a
     slow idle bob so the page keeps feeling alive at rest.
     ------------------------------------------------------------ */
  document.addEventListener('DOMContentLoaded', () => {
    // Runs after the FAB block above has mounted — both are
    // DOMContentLoaded listeners, so order follows source order.
    requestAnimationFrame(() => {
      const groups = [
        document.querySelectorAll('.nav-links a'),
        document.querySelectorAll('.pill'),
        document.querySelectorAll('.stat-tile'),
        document.querySelectorAll('.card'),
        document.querySelectorAll('.card .btn, .card a.btn'),
        document.querySelectorAll('.linkcard'),
        document.querySelectorAll('.fab-main')
      ];

      let i = 0;
      groups.forEach(list => {
        list.forEach(el => {
          el.style.setProperty('--fd', `${i * 55}ms`);
